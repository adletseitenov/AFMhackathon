...
HANDCRAFTED_SIGNALS = [
    ...
    {
        "name": "pyramid_recruit",
        "pattern": re.compile(
            r"(бинарн\w*\s+матриц\w*|"
            r"пожизненн\w*\s+выплат\w*|"
            r"реинвест\w*\s+(удва\w*|капитал)|"
            r"оборот\w*\s+(всей\s+)?(тво\w+\s+)?сет\w*|структур\w*\s+(партнёр\w*|сет\w*)|"
            r"команд\w*\s+(лидер\w*|основател\w*)|партнёр\w*[-\s]?основател\w*|"
            r"\d\s*уровн\w*\s+(реферал\w*|глубин\w*|структур\w*)|уровн\w*\s+реферальн\w*|"
            r"выш(ел|ла|ёл)\s+на\s+(стабильн\w*\s+|пассивн\w*\s+)?доход|"
            r"мест[ао]?\s+\w{0,12}\s*(в\s+(матриц\w*|пуле|структур\w*|команд\w*)\s+)?(тают|разбер\w*)|"
            r"вход\s+(в\s+)?(проект|пул|матриц\w*|команд\w*|структур\w*)|"
            r"вложил\s+\S+\s+(снял|вывел|забрал)|"
            r"өмір\s+бойы\s+төлем\w*|матрица\w*\s+орын\w*|орын\w*\s+азай\w*|"
            r"құрылым\w*\s+құр\w*|қатысушы\w*\s+тарт\w*|желі\w*\s+кеңей\w*|"
            r"касс\w*\s+взаимопомощ\w*|взаимопомощ\w*|круг\s+замкн\w*|"
            r"дар(им|ишь)\s+(друг\s+друг\w*|одному|деньги)|тебе\s+дар(ят|уют))",
            re.I | re.U,
        ),
        "category_hint": "pyramid",
        "evidence_ru": "Вербовка в структуру/матрицу (HYIP/MLM): уровни, пожизненные выплаты, реинвест",
    },
    {
        "name": "promo_code",
        "pattern": re.compile(
    ...
    },
    {
        ...
        "pattern": re.compile(
            ...
            r"перевед(и|ите)\s+\w{0,12}\s*на\s+(резервн\w*|безопасн\w*)\s+сч[её]т|"
            r"установит\w*\s+приложени\w*\s+по\s+ссылк\w*|"
            r"предоплат\w*|"
            r"подтвердит\w*\s+(данные\s+)?карт\w*|"
            r"ақысын\s+төле\w*|жеткізу\s+ақы\w*|сілтеме\s+арқылы\s+төле\w*|"
            r"смс\s+код\w*\s+(айт|жібер)\w*|код\w*\s+айтыңыз|"
            r"карта\w*\s+бұғаттал\w*|шот\w*\s+бұғаттал\w*)",
            re.I | re.U,
        ),
        "category_hint": "fraud",
    ...
    },
    {
        ...
        "pattern": re.compile(
            ...
            r"ұтып\s+алдыңыз|сіз\s+ұтт\w*|жүлдені?\s+ал\w*|ұтыс\w*\s+(сіздік|сізге|сізде))",
            re.I | re.U,
        ),
        "category_hint": "fraud",
    ...
    },
    {
        ...
        "pattern": re.compile(
            ...
            r"(монет\w*|токен\w*)\s+\w{0,15}(взлет\w*|вырастет|x\d+)|x\d+\s+на\s+\w{0,8}\s*недел\w*|"
            r"инсайд\w*\s+от\s+команд\w*|закупайт\w*\s+(сейчас|пока)|"
            r"подключ(и|ите)\s+кошел\w*|seed[-\s]?фраз\w*|сид[-\s]?фраз\w*)",
            re.I | re.U,
        ),
        "category_hint": "fraud",
